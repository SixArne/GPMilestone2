...
{
	/*const float dtTotal = sceneContext.GetGameTime()->GetTotal();
	const float dt = sceneContext.GetGameTime()->GetElapsed();


	if (m_IsFlipped)
	{
		RotateDegrees(0.f, -90.f * dtTotal, 0.f);
	}
	else
	{
		RotateDegrees(0.f, 90.f * dtTotal, 0.f);
	...
	const float movementSpeed{ 10.f * dt };
	if (sceneContext.GetInput()->IsActionTriggered(InputIDs::MoveLeft))
	{
		translation.x -= movementSpeed;
	}
	if (sceneContext.GetInput()->IsActionTriggered(InputIDs::MoveRight))
	{
		translation.x += movementSpeed;
	}

	auto currPosition = XMLoadFloat3(&GetPosition());
	auto deltaPosition = XMLoadFloat3(&translation);

	XMFLOAT3 result{};
	XMStoreFloat3(&result, currPosition + deltaPosition);
