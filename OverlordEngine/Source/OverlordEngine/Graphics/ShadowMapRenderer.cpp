...
void ShadowMapRenderer::Initialize()
{
	...
	auto hrs = m_pShadowRenderTarget->Create(rtdesc);
	if (!FAILED(hrs))
	{
	...
	constexpr auto staticType = static_cast<int>(ShadowGeneratorType::Static);
	constexpr auto skinnedType = static_cast<int>(ShadowGeneratorType::Skinned);
	m_GeneratorTechniqueContexts[staticType] = m_pShadowMapGenerator->GetTechniqueContext(staticType);
	m_GeneratorTechniqueContexts[skinnedType] = m_pShadowMapGenerator->GetTechniqueContext(skinnedType);
}
...
void ShadowMapRenderer::UpdateMeshFilter(const SceneContext& sceneContext, MeshFilter* pMeshFilter) const
{
	...
	auto technique = (isSkinned) 
		? m_GeneratorTechniqueContexts[skinnedType] 
		: m_GeneratorTechniqueContexts[staticType];
...
void ShadowMapRenderer::Begin(const SceneContext& sceneContext)
{
	...
	//1. Making sure that the ShadowMap is unbound from the pipeline as ShaderResourceView (SRV) is important, because we cannot 
	// use the same resource as a ShaderResourceView (texture resource inside a shader) and a RenderTargetView
	//  (target everything is rendered too) at the same time. In case this happens, you'll see an error in the output of
	//  visual studio - warning you that a resource is still bound as a SRV and cannot be used as an RTV.
	// 
	//	-> Unbinding an SRV can be achieved using DeviceContext::PSSetShaderResource [I'll give you the implementation for free]
	//  - double check your output because depending on your usage of ShaderResources, the actual slot the ShadowMap is using 
	// can be different, but you'll see a warning pop-up with the correct slot ID in that case.
	...
	//		*nearZ>0.1f
	//		*farZ>500.f
	auto projectionMatrix = DirectX::XMMatrixOrthographicLH(100.f * sceneContext.aspectRatio, 100.f, 0.1f, 500.f);
	...
	DirectX::XMVECTOR lightDirection{ XMLoadFloat4(&light.direction) };
	DirectX::XMVECTOR lightPosition{ XMLoadFloat4(&light.position) };

	auto viewMatrix = DirectX::XMMatrixLookAtLH(lightPosition, lightPosition + lightDirection, { 0,1,0,0 });
	XMStoreFloat4x4(&m_LightVP, DirectX::XMMatrixMultiply(viewMatrix, projectionMatrix));
