...
LightingResult DoPointLighting(Light light, Material mat, float3 V, float3 N, float3 P)
{
	...
	result.Specular = DoSpecular(light, mat, L, V, N) * light.Intensity * attenuation;
...
LightingResult DoSpotLighting(Light light, Material mat, float3 V, float3 N, float3 P)
{
	...
	result.Diffuse = DoDiffuse(light, L, N) * light.Intensity * coneFalloff * attenuation;
	result.Specular = DoSpecular(light, mat, L, V, N) * light.Intensity * coneFalloff * attenuation;
